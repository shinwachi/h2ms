...
docker run -e POSTGRES_PASSWORD=root -e POSTGRES_USER=postgres -e POSTGRES_DB=h2msauthdb -d -p 5433:5432 --name postgresauth postgres
...
docker rm postgres
docker stop postgresauth
docker rm postgresauth
docker-compose up